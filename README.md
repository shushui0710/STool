...
> - **反修改保护识别**：数值改完立刻被还原 / 改了没反应时，诊断这个地址是被周期回滚、
>   只读页保护、副本覆盖还是代码层还原，按可行性给出可直接执行的应对方案（强制写入 /
>   自适应锁值 / 改数据源 / 代码补丁），让改动能稳定生效；识别到在线反作弊只给风险提示
...
打开图形界面的"💾 存档编辑"页。这一页是**搜索优先**的：大存档（几千个开关 / 变量）也能几秒定位到要改的那一项。

1. **打开存档文件** —— 直接把存档文件**拖进窗口**最方便；也支持手动选择或按路径加载，程序会自动识别格式
2. **搜索定位（主入口）** —— 输入键名、数字或文字（比如金币数 1000、角色名），下面立刻列出所有匹配位置；
   每行显示"路径 + 当前值 + 类型"，值按类型着色（文本文/数值/布尔各一色）。结果列表只渲染可见行，
   上千条也不卡；点一行即选中
3. **直接修改** —— 选中后下面的「✏ 修改字段」卡片会显示路径、类型、当前值；填新值回车或点「应用」即生效，
   还带「重置 / +1 / −1 / 设为 true/false」等快捷按钮
4. **保存回写** —— 一键写回原文件，改之前自动备份为 `*.stool.bak`；也可以导出/导入 JSON 做备份或分享修改
5. **原始 JSON 结构**（可选）—— 页面底部是一个**默认折叠**的辅助视图，用于排错或精确定位，
   日常改数值用上面的搜索即可
...
**方式二·五：数值改完立刻被还原？（反修改保护识别）**

有些单机游戏对关键数值做了"反修改"处理：内存写入看起来成功了，但**过一瞬又变回去**，
或者**根本不生效**。这时别急着换地址，先让 STool 诊断一下这个地址到底被什么保护着。

在方式二·五区块填上出问题的地址（点方式二命中列表里的"诊断"按钮会自动带过来），
选好类型和探测值，点"🔬 诊断这个地址"。它会**临时写入探测值再立即恢复原值**（不做任何持久修改），
然后给出四条事实：

| 事实 | 它说明什么 |
|---|---|
| **回滚判定** | 稳定 / 立即回滚 / 周期性回滚（附实测间隔）/ 延迟回滚。区分"改了没反应"是地址不对还是被还原 |
| **改写判定** | 写入值与读回值的 XOR / 偏移 / 常量关系。有这种关系说明该地址是"派生/缓存"，写它没用，得改真正的数据源 |
| **页保护** | 只读 / Guard / 私有 vs 映像。这类页普通写入会被系统拒绝，**用"强制写入"临时解除保护再写** |
| **保护机制线索** | 已加载模块里的加壳（Themida/VMProtect/UPX…）/ 本地 DRM / 在线反作弊；以及主程序导入的反调试 API |

旁边的 **"🗺 页保护分布"** 是**只读**操作（不写内存、不需要确认）：遍历整个进程的内存区域，
报出有多少只读页 / Guard 页 / **"可写且可执行"页**（正常程序极少，是加壳或自修改代码的常见特征），
并可展开看每个区域的基址、大小、保护、类型。

最后按可行性评级给出**应对方案清单**，能直接执行的就带按钮：

1. **强制写入** —— 页只读 / Guard 时，临时改页为可写、写完立刻恢复原保护；
2. **自适应锁值** —— 周期性回滚时，按实测间隔自动设好写入周期（例如每 1 毫秒写回一次）反复压制；
3. **写入这个数据源** —— 诊断会拿同值副本逐个反测（只试"可写且不可执行"的普通数据页，
   最多 24 个），找出**写它会让目标地址跟着变**的那个副本 —— 它才是游戏真正采纳的"源"，
   写它、锁它才有用（尤其适合"写原地址没用 / 同值副本几十个"的情况）；
4. **代码补丁** —— 在可执行区定位"写到这个地址"的立即数指令（还原指令落点），改掉它根治；
5. **替代路线** —— 改存档 / 改脚本 / 引擎调试接口（比改内存稳，不掉、不受校验影响）；
6. **在线反作弊** —— 识别到 EasyAntiCheat / BattlEye / Vanguard / ACE 等只给风险提示，**不提供绕过**。

命令行等价写法：

```bash
stool-cli guard <pid|进程名> --opt:addr=0x1234ABCD --opt:type=i32   # 诊断单个地址
stool-cli guard-regions <pid|进程名> --opt:max=20                   # 看整块内存的页保护分布
stool-cli guard <pid> --opt:addr=0x1234 --opt:type=i32 --opt:no-mirror --opt:no-code   # 跳过耗时项，只看回滚判定
```

可选参数：`--opt:probe=值`（指定探测值，默认自动构造）、`--opt:window=ms`（每轮存活观察窗口）、
`--opt:rounds=n`（探测轮数）、`--opt:no-mirror`（跳过同值副本扫描）、`--opt:no-code`（跳过代码段扫描）。
...
# 反修改保护：数值改完被还原 / 改了没反应时，诊断地址被什么保护着并给应对方案
stool-cli guard <pid|进程名> --opt:addr=0x1234ABCD --opt:type=i32
stool-cli guard <pid> --opt:addr=0x1234 --opt:type=f32 --opt:probe=9999 --opt:window=60 --opt:rounds=5
stool-cli guard <pid> --opt:addr=0x1234 --opt:type=i32 --opt:no-mirror --opt:no-code   # 只看回滚判定
stool-cli guard-regions <pid|进程名> --opt:max=20   # 看整块内存的页保护分布（找只读/可执行页）
...
- **243 项单元测试**（内置在 `src/` 各模块）：各格式解析/回环、存档编辑、文本提取回填、
  ...
  方案生成 / 页保护汇总 / 同值副本反测候选筛选 / PE 导入表解析 / 报告渲染）……
...
「不回滚 / 周期回滚（10ms）/ 高频回滚（1ms）/ 只读页 / 只读页+自写回」，
`scripts/_guard_smoke.sh` 依次起靶子、用 `stool-cli guard` 诊断。实测结果：
`none` → 判定「稳定（非内存还原）」；`periodic` / `fast` → 判定「周期性回滚」并给出 1ms 锁值方案；
`readonly` → 判定「页只读，用强制写入」；`constant` → 判定「页只读 + 周期回滚」。
这是**真机跨进程读写**（不是合成数据），覆盖 `OpenProcess` / `ReadProcessMemory` / `WriteProcessMemory` /
`VirtualQueryEx` / `VirtualProtectEx` 全链路与各类错误路径。
