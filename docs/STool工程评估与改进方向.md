...
>
> **文档性质：历史记录，保留当时的原始判断，不随之后的重构回改。**
> 其中两条判断后来被推翻，读到时请按这里更正：
> ① §"明确不做"里的「**不引入 GUI 框架替换（egui 够用）**」——2026-09 已推翻，
> 界面整体换成 Tauri v2（`docs/TAURI重构方案.md`），并进一步删除了 egui 版；
> ② 全文提到的 `gui.rs` / `gui.rs:991` / `gui/mod.rs` / `gui/pages/*.rs` 等路径**已不存在**，
> 现为 `stool-tauri/src/js/pages/*.js` + `stool-tauri/src-tauri/src/cmd.rs`。
...
- ~~不引入 GUI 框架替换（egui 够用，重写 UI 成本远大于收益）。~~
  **此条已被推翻（2026-09）**：界面换成了 Tauri v2，且旧 egui 版已删除
  —— 见 `docs/TAURI重构方案.md` 文首状态说明。当时的成本估计偏悲观，
  而「按任务而非技术机制重排导航」这类易用性改动在 WebView 里做起来远比在自绘控件里便宜。
